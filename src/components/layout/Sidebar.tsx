        ...
        {MENU_ITEMS.map((item) => (
          <div key={item.id}>
            ...
            {item.id === "mods" && showDropdown && (
              <div className="mt-1 ml-3 max-h-64 overflow-y-auto space-y-0.5">
                {characters.map((character) => (
                  <button
                    key={character.id}
                    onClick={() => onSelectCharacter(character.id)}
                    className={`w-full rounded-lg flex items-center gap-2 px-2.5 py-2 transition-all duration-200 ${
